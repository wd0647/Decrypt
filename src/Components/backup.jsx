...
import raw from './collection.txt';
...
class DataTable extends React.Component{
	...
	constructor(props) {
		...
		fetch(raw)
		  .then(r => r.text())
		  .then(text => {
		  			console.log(JSON.parse(text)[0]['cryptoLabel'])
		  			const parsedData = JSON.parse(text).sort(GetSortOrder('today_yield')).reverse()
		  			console.log( parsedData)
						this.tokenList = parsedData.map((token,index) => {
								console.log(token['cryptoLabel'])
								return(
